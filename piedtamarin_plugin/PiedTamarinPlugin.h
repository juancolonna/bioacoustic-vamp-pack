/**
 * PiedTamarinPlugin.h — VAMP plugin header for pied tamarin sounds detection.
 *
 * This plugin accumulates audio samples during processing, writes them to a
 * temporary WAV file, and invokes the PiedTamarin inference script (piedtamarin_run.py)
...
class PiedTamarinPlugin : public Vamp::Plugin {
public:
    PiedTamarinPlugin(float inputSampleRate);
    virtual ~PiedTamarinPlugin();
...
private:
    ...
    // Holds a single PiedTamarin detection parsed from JSON output
    struct Detection {
    ...
    // Parses the JSON array returned by piedtamarin_run.py

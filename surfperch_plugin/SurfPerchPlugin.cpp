/**
 * SurfPerchPlugin.cpp — VAMP plugin implementation for reef coral sounds detection with SurfPerch.
 *
 * Processing strategy:
 *   1. process()              — accumulates all input samples into m_audioBuffer.
 *   2. getRemainingFeatures() — writes a temporary WAV file, invokes surfperch_run.py
 *                               via popen(), parses the JSON output, and returns
 *                               labeled VAMP features with timestamps.
 *
 * The Python subprocess (surfperch_run.py) is executed via `uv run`, which
...
SurfPerchPlugin::SurfPerchPlugin(float inputSampleRate)
    ...
    , m_stride(5.0f)
{
    ...
    m_scriptPath = pluginDir + "/surfperch_run.py";
    m_wavPath    = pluginDir + "/surfperch_analysis.wav";
}

SurfPerchPlugin::~SurfPerchPlugin() {}

// ── Initialisation ───────────────────────────────────────────────────────────

bool SurfPerchPlugin::initialise(size_t channels, size_t stepSize, size_t blockSize) {
...
Plugin::FeatureSet
SurfPerchPlugin::process(const float* const* inputBuffers,
...
Plugin::FeatureSet SurfPerchPlugin::getRemainingFeatures() {
    FeatureSet output;
...
void SurfPerchPlugin::writeWAV(const std::string& path,
                               const float* samples,
                               int n,
                               int sr) const
{
...
std::vector<SurfPerchPlugin::Detection>
SurfPerchPlugin::parseJSON(const std::string& json) const
{
...
size_t SurfPerchPlugin::getPreferredBlockSize() const { return 256; }
size_t SurfPerchPlugin::getPreferredStepSize()  const { return 256; }

// ── Configurable parameters ──────────────────────────────────────────────────

Plugin::ParameterList SurfPerchPlugin::getParameterDescriptors() const {
...
float SurfPerchPlugin::getParameter(std::string id) const {
...
void SurfPerchPlugin::setParameter(std::string id, float value) {
...
Plugin::InputDomain SurfPerchPlugin::getInputDomain() const {
    return TimeDomain;
}
...
vampGetPluginDescriptor(unsigned int version, unsigned int index) {
    ...
    static Vamp::PluginAdapter<SurfPerchPlugin> adapter;

/**
 * SurfPerchPlugin.h — VAMP plugin header for reef coral sounds detection using SurfPerch.
 *
 * This plugin accumulates audio samples during processing, writes them to a
 * temporary WAV file, and invokes the SurfPerch inference script (surfperch_run.py)
...
class SurfPerchPlugin : public Vamp::Plugin {
public:
    SurfPerchPlugin(float inputSampleRate);
    virtual ~SurfPerchPlugin();
...
private:
    ...
    // Holds a single SurfPerch detection parsed from JSON output
    struct Detection {
    ...
    // Parses the JSON array returned by surfperch_run.py
    ...
    std::string        m_scriptPath;    // path to surfperch_run.py
